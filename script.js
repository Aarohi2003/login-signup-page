...
// alerts in a web page in signup
...
document.getElementById("email").addEventListener("input", function () {
  ...
  if (!emailRegex.test(email)) {
    ...
    if (email.indexOf("@") === 0) {
      ...
      emailError.textContent = "@ cannot come in the beginning";
    ...
    else if (!/\.(com|co)$/i.test(email)) {
      emailError.textContent = "Email must end with '.com' or '.co'.";
    }

    else {
      emailError.textContent = "(.) is used in the wrong file format";
    }
    return;
  }
  emailError.textContent = "";
...
document.getElementById("email1").addEventListener("input", function () {
  ...
  if (!emailRegex.test(email1)) {
    ...
    if (email1.indexOf("@") === 0) {
      ...
      emailError1.textContent = "@ cannot come in the beginning";

    }
    else if (!email1.includes("@")) {
      emailError1.textContent = "Email must contain '@'.";
    }

    else if (!/\.(com|co)$/i.test(email1)) {
      emailError1.textContent = "Email must end with '.com' or '.co'.";
    }

    else {
      emailError1.textContent = "(.) is used in the wrong file format";
    }
    return;
  }
  emailError1.textContent = "";
